		...
		public async Task<FileResult> WageRegisterNewExcel(int WAG_Id, int FRM_Id)
		{
			...
			using (var fs = new FileStream(Path.Combine(newPath, fileName), FileMode.Create, FileAccess.Write))
			{
				...
				foreach (var client in wage_Process.Wage_Process_Clients.OrderBy(c => c.CLI.CLI_RegisteredOn))
				{
					...
					decimal Final_TotalEarnedBasic = 0M, Final_TotalEarnedHRA = 0M, Final_TotalEarnedLeaveAndPH = 0M, Final_TotalGrossWagePayble = 0M, Final_TotalAdvance = 0M, Final_TotalProfTax = 0M, Final_TotalPF = 0M, Final_TotalESIC = 0M, Final_TotalOtherDeduction = 0M, Final_TotalNetWagesPaid = 0M;
					...
					foreach (int CRI_Id in CRI_Ids)
					{
						...
						cell42.SetCellValue("Earned Leave&PH");
						cell42.CellStyle = styleGrey25;
						excelSheet.AddMergedRegion(new CellRangeAddress(DesCount, DesCount + 1, cellCount, cellCount));

						cellCount = cellCount + 1;
						ICell cell43 = row.CreateCell(cellCount);
						cell43.SetCellValue("Earned CCA");
						cell43.CellStyle = styleGrey25;
						excelSheet.AddMergedRegion(new CellRangeAddress(DesCount, DesCount + 1, cellCount, cellCount));
						...
						decimal TotalEarnedBasic = 0M, TotalEarnedHRA = 0M, TotalEarnedLeaveAndPH = 0M, TotalGrossWagePayble = 0M, TotalAdvance = 0M, TotalProfTax = 0M, TotalPF = 0M, TotalESIC = 0M, TotalOtherDeduction = 0M, TotalNetWagesPaid = 0M;
						...
						foreach (Wage_Register wage_register in wage_registers)
						{
							...
							cellEmpCount = cellEmpCount + 1;
							ICell cellEmp32 = row.CreateCell(cellEmpCount); //Earned Leave&PH
							TotalEarnedLeaveAndPH = TotalEarnedLeaveAndPH + wage_register.WAR_LeaveAndPH_Calculated;
							cellEmp32.SetCellValue(Math.Round(wage_register.WAR_LeaveAndPH_Calculated, MidpointRounding.AwayFromZero).ToString());
							cellEmp32.CellStyle = styleGrey25;

							cellEmpCount = cellEmpCount + 1;
							ICell cellEmp33 = row.CreateCell(cellEmpCount); //Earned CCA
							cellEmp33.SetCellValue("-");
							cellEmp33.CellStyle = styleGrey25;
						...
						cellTot4.SetCellValue(Math.Round(TotalEarnedLeaveAndPH, MidpointRounding.AwayFromZero).ToString());
						cellTot4.CellStyle = styleTotal;
						ICell cellTot5 = row.CreateCell(totalCount + 4);
						...
						Final_TotalEarnedLeaveAndPH = Final_TotalEarnedLeaveAndPH + TotalEarnedLeaveAndPH;
					...
					fcellTot3.SetCellValue("Total Earned Leave&PH");
					fcellTot3.CellStyle = styleTotal;
					ICell fcellTot3s = frowSecond.CreateCell(ftotalCount);
					fcellTot3s.SetCellValue(Math.Round(Final_TotalEarnedHRA, MidpointRounding.AwayFromZero).ToString());
					fcellTot3s.CellStyle = styleTotal;
					ftotalCount = ftotalCount + 1;

					ICell fcellTot4 = frow.CreateCell(ftotalCount);
					fcellTot4.SetCellValue("Total Earned CCA");
					fcellTot4.CellStyle = styleTotal;
					ICell fcellTot4s = frowSecond.CreateCell(ftotalCount);
					fcellTot4s.SetCellValue("0");
					fcellTot4s.CellStyle = styleTotal;
					ftotalCount = ftotalCount + 1;
		...
		public async Task<FileResult> WageRegisterExcel(int WAG_Id, int FRM_Id)
		{
			...
			using (var fs = new FileStream(Path.Combine(newPath, fileName), FileMode.Create, FileAccess.Write))
			{
				...
				foreach (var client in wage_Process.Wage_Process_Clients.OrderBy(c => c.CLI.CLI_RegisteredOn))
				{
					...
					decimal Final_TotalBasic = 0M, Final_TotalDA = 0M, Final_TotalHRA = 0M, Final_TotalLeaveAndPH = 0M, Final_TotalOT = 0M, Final_TotalGrossTotal = 0M, Final_TotalPF = 0M, Final_TotalESIC = 0M;
					...
					foreach (int CRI_Id in CRI_Ids)
					{
						...
						cell13.SetCellValue("Total Leave&PH");
						...
						if (!cri.CRI_OT_Calculate_Payableday)
						{
							...
							cell = cell + 1;
							ICell cell14 = row.CreateCell(cell);
							cell14.SetCellValue("OT Amount");
							cell14.CellStyle = styleGrey40;
						}
						int count = cell + 1;
						ICell cell15 = row.CreateCell(cell + 1);
						cell15.SetCellValue("Gross Total");
						cell15.CellStyle = styleGrey40;
						ICell cell16 = row.CreateCell(cell + 2);
						cell16.SetCellValue("PF");
						cell16.CellStyle = styleGrey50;
						ICell cell17 = row.CreateCell(cell + 3);
						cell17.SetCellValue("ESIC");
						...
						ICell cell18 = row.CreateCell(cellNext1);
						cell18.SetCellValue("Advance Installment");
						...
						ICell cell19 = row.CreateCell(cellLWFNext);
						cell19.SetCellValue("Deduct Total");
						cell19.CellStyle = styleGrey50;
						ICell cell20 = row.CreateCell(cellLWFNext + 1);
						cell20.SetCellValue("Final Amount");
						cell20.CellStyle = styleGrey80;

						ICell cell21 = row.CreateCell(cellLWFNext + 2);
						cell21.SetCellValue("Signaure");
						excelSheet.SetColumnWidth(cellLWFNext + 2, (int)((20 + 0.72) * 256));
						cell21.CellStyle = styleGrey80;
						#endregion

						DesCount++;
						var wage_registers = wgs.Where(wg => wg.CRI_Id.Equals(cri.CRI_Id)).ToList();
						double TotalPaybleDays = 0, TotalOTHrs = 0;
						decimal TotalBasic = 0M, TotalDA = 0M, TotalHRA = 0M, TotalLeaveAndPH = 0M, TotalOT = 0M, TotalGrossTotal = 0M, TotalPF = 0M, TotalESIC = 0M, TotalProfTax = 0m, TotalRevenue = 0M, TotalCanteenFacility = 0M, TotalAdvance = 0M, TotalDeduct = 0M, TotalFinal = 0M;
						decimal TotalOutStation = 0M, TotalAttendance = 0M, TotalNightshift = 0M, TotalPerformance = 0M, TotalLWF = 0M;
						decimal TotalAllowance1 = 0M, TotalAllowance2 = 0M, TotalAllowance3 = 0M, TotalAllowance4 = 0M, TotalAllowance5 = 0M, TotalAllowance6 = 0M, TotalAllowance7 = 0M, TotalAllowance8 = 0M, TotalAllowance9 = 0M, TotalAllowance10 = 0M;
						int srno = 0;
						decimal[] otherAllowances = new decimal[cri.Client_Requirement_Allowances.Count()];
						foreach (Wage_Register wage_register in wage_registers)
						{
							...
							ICell cellEmp13 = row.CreateCell(13);
							cellEmp13.SetCellValue(Math.Round(wage_register.WAR_LeaveAndPH_Calculated, MidpointRounding.AwayFromZero).ToString());
							TotalLeaveAndPH = TotalLeaveAndPH + Math.Round(wage_register.WAR_LeaveAndPH_Calculated, MidpointRounding.AwayFromZero);
							...
							if (!cri.CRI_OT_Calculate_Payableday)
							{
								...
								ICell cellEmp14 = row.CreateCell(cellNxt);
								cellEmp14.SetCellValue(Math.Round(wage_register.WAR_OverTime_Calculated, MidpointRounding.AwayFromZero).ToString());
							...
							ICell cellEmp15 = row.CreateCell(cellNxt);
							cellEmp15.SetCellValue(Math.Round(wage_register.WAR_GrossTotal, MidpointRounding.AwayFromZero).ToString());
							TotalGrossTotal = TotalGrossTotal + Math.Round(wage_register.WAR_GrossTotal, MidpointRounding.AwayFromZero);
							cellEmp15.CellStyle = styleGrey40;
							ICell cellEmp16 = row.CreateCell(cellNxt + 1);
							cellEmp16.SetCellValue(Math.Round(wage_register.WAR_PF_Calculated, MidpointRounding.AwayFromZero).ToString());
							TotalPF = TotalPF + Math.Round(wage_register.WAR_PF_Calculated, MidpointRounding.AwayFromZero);
							cellEmp16.CellStyle = styleGrey50;
							ICell cellEmp17 = row.CreateCell(cellNxt + 2);
							cellEmp17.SetCellValue(Math.Round(wage_register.WAR_ESIC_Calculated, MidpointRounding.AwayFromZero).ToString());
							...
							ICell cellEmp18 = row.CreateCell(cellNxt1 + 1);
							...
							cellEmp18.SetCellValue(WAR_Advance_Amount.ToString());
							...
							ICell cellEmp19 = row.CreateCell(cellNext2);
							cellEmp19.SetCellValue(DeductTotal.ToString());
							cellEmp19.CellStyle = styleGrey50;
							TotalDeduct = TotalDeduct + DeductTotal;
							ICell cellEmp20 = row.CreateCell(cellNext2 + 1);
							cellEmp20.SetCellValue(Math.Round(wage_register.WAR_FinalTotal.Value, MidpointRounding.AwayFromZero).ToString());
							...
							ICell cellEmp21 = row.CreateCell(cellNext2 + 2);
							cellEmp21.SetCellValue("");
							cellEmp21.CellStyle = styleGrey80;
						...
						ICell cellTot5 = row.CreateCell(totalsCell_Index + 5);
						cellTot5.SetCellValue(Math.Round(TotalLeaveAndPH, MidpointRounding.AwayFromZero).ToString());
						cellTot5.CellStyle = styleGrey40;

						int cellAllow = totalsCell_Index + 5;
						...
						if (!cri.CRI_OT_Calculate_Payableday)
						{
							...
							totalsCell_Index = totalsCell_Index + 1;
							ICell cellTot6 = row.CreateCell(totalsCell_Index);
							cellTot6.CellStyle = styleGrey40;
							cellTot6.SetCellValue(Math.Round(TotalOT, MidpointRounding.AwayFromZero).ToString());
						}

						ICell cellTot7 = row.CreateCell(totalsCell_Index + 1);
						cellTot7.SetCellValue(Math.Round(TotalGrossTotal, MidpointRounding.AwayFromZero).ToString());
						cellTot7.CellStyle = styleGrey40;
						ICell cellTot8 = row.CreateCell(totalsCell_Index + 2);
						cellTot8.SetCellValue(Math.Round(TotalPF, MidpointRounding.AwayFromZero).ToString());
						cellTot8.CellStyle = styleGrey50;
						ICell cellTot9 = row.CreateCell(totalsCell_Index + 3);
						cellTot9.SetCellValue(Math.Round(TotalESIC, MidpointRounding.AwayFromZero).ToString());
						...
						if (cri.CRI_ProfessionalTax)
						{
							...
							cellTot10.SetCellValue(Math.Round(TotalProfTax, MidpointRounding.AwayFromZero).ToString());
						...
						if (cri.CRI_RevenueDeduction)
						{
							...
							cellTot11.SetCellValue(Math.Round(TotalRevenue, MidpointRounding.AwayFromZero).ToString());
						...
						if (cri.CRI_CanteenFacility)
						{
							...
							totalsCell_Index++;
							ICell cellTot12 = row.CreateCell(totalsCell_Index);
							cellTot12.SetCellValue(Math.Round(TotalCanteenFacility, MidpointRounding.AwayFromZero).ToString());
							cellTot12.CellStyle = styleGrey50;
						}

						ICell cellTot13 = row.CreateCell(totalsCell_Index + 1);
						cellTot13.SetCellValue(Math.Round(TotalAdvance, MidpointRounding.AwayFromZero).ToString());
						...
						ICell cellTot14 = row.CreateCell(totalsCell_Index);
						cellTot14.SetCellValue(Math.Round(TotalDeduct, MidpointRounding.AwayFromZero).ToString());
						cellTot14.CellStyle = styleGrey50;
						ICell cellTot15 = row.CreateCell(totalsCell_Index + 1);
						cellTot15.SetCellValue(Math.Round(TotalFinal, MidpointRounding.AwayFromZero).ToString());
						cellTot15.CellStyle = styleGrey80;

						ICell cellTot16 = row.CreateCell(totalsCell_Index + 2);
						cellTot16.SetCellValue("");
						cellTot16.CellStyle = styleGrey80;

						Final_TotalPaybleDays += TotalPaybleDays;
						Final_TotalBasic += TotalBasic;
						Final_TotalDA += TotalDA;
						Final_TotalHRA += TotalHRA;
						Final_TotalLeaveAndPH += TotalLeaveAndPH;
					...
					ICell fcellTot5 = frow.CreateCell(ftotalCount + 5);
					fcellTot5.SetCellValue("Total Leave&PH");
					fcellTot5.CellStyle = styleGrey40;
					ICell fcellTot5s = frowSecond.CreateCell(ftotalCount + 5);
					fcellTot5s.SetCellValue(Math.Round(Final_TotalLeaveAndPH, MidpointRounding.AwayFromZero).ToString());
					fcellTot5s.CellStyle = styleGrey40;

					ftotalCount = ftotalCount + 6;
		...
		public JsonResult Calculate_PF_ESIC([FromBody] CalculationEditVM CalculationEditVM)
		{
			...
			if (CalculationEditVM != null)
			{
				...
				decimal PFsum = Math.Round(GetAmountBasedOnFormula_Edit(
								CalculationEditVM.CRI_PF_Formula, CalculationEditVM.WAR_Basic_Calculated, CalculationEditVM.CRI_DA_Calculated, CalculationEditVM.CRI_HRA_Calculated, CalculationEditVM.CRI_LeaveAndPH_Calculated,
				...
				decimal ESICsum = Math.Round(GetAmountBasedOnFormula_Edit(
							   CalculationEditVM.CRI_ESIC_Formula, CalculationEditVM.WAR_Basic_Calculated, CalculationEditVM.CRI_DA_Calculated, CalculationEditVM.CRI_HRA_Calculated, CalculationEditVM.CRI_LeaveAndPH_Calculated,
				...
				if (wageRegister.CRI.CRI_PF_ApplyMAX.HasValue && wageRegister.CRI.CRI_PF_ApplyMAX.Value > 0)
				{
					if (PFsum >= wageRegister.CRI.CRI_PF_ApplyMAX)
					{
						calculated.WAR_PF_Calculated = Math.Round(Decimal.Multiply(wageRegister.CRI.CRI_PF_ApplyMAX.Value, CalculationEditVM.CRI_PF_Percentage) / 100, MidpointRounding.AwayFromZero);
					}
					else
					{
					...
					if (!wageRegister.CRI.CRI_OT_Calculate_Payableday)
					{
						...
						else if (wageRegister.CRI.CRI_OT_Formula != null)
						{
							...
							decimal OTsum = Math.Round(GetAmountBasedOnFormula_Edit(
											CalculationEditVM.CRI_OT_Formula, CalculationEditVM.WAR_Basic_Calculated, CalculationEditVM.CRI_DA_Calculated, CalculationEditVM.CRI_HRA_Calculated, CalculationEditVM.CRI_LeaveAndPH_Calculated,
											CalculationEditVM.CalculatedAllowanceVM, CalculationEditVM.totalWorkingDays, CalculationEditVM.totalPaybleDays,
											CalculationEditVM.WAR_OverTime_Calculated, CalculationEditVM.WAR_Outstation_Allowance_Calculated, CalculationEditVM.WAR_Attendance_Allowance_Calculated,
											CalculationEditVM.WAR_Nightshift_Allowance_Calculated, CalculationEditVM.WAR_Performance_Allowance_Calculated,
											 CalculationEditVM.WAR_Allowance_Calculated_1, CalculationEditVM.WAR_Allowance_Calculated_2, CalculationEditVM.WAR_Allowance_Calculated_3, CalculationEditVM.WAR_Allowance_Calculated_4, CalculationEditVM.WAR_Allowance_Calculated_5, CalculationEditVM.WAR_Allowance_Calculated_6, CalculationEditVM.WAR_Allowance_Calculated_7, CalculationEditVM.WAR_Allowance_Calculated_9, CalculationEditVM.WAR_Allowance_Calculated_9, CalculationEditVM.WAR_Allowance_Calculated_10), MidpointRounding.AwayFromZero);
