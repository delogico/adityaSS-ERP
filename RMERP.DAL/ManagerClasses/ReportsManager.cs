...
namespace RMERP.DAL.ManagerClasses
{
    public class ReportsManager
    {
        ...
        public List<PayTaxReportVM> PayTaxReports(int WAG_Id)
        {
            ...
            List<Wage_Register> wage_Registers = wages.Where(m => m.CRI.CRI_ProfessionalTax == true).ToList();
            foreach (var item in wage_Registers.Select(m => new { m.CLI_Id, m.CLI.CLI_Name }).Distinct())
            {
                ...
                foreach (var emp in register)
                {
                    ...
                    if (emp.CRI.CRI_ProfessionalTax == true)
                    {
        ...
        public List<BankReportVM> IDBI_TO_IDBI_AdvanceReports(int WAG_Id, int FRM_Id)
        {
            ...
            List<Employee_Advance> employee_Advances = GetIDBI_TO_IDBIEmployeeAdvances(new DateTime(wage_Process.WAG_Month.Year, wage_Process.WAG_Month.Month, wage_Process.WAG_Month.Day), FRM_Id);
            ...
            foreach (var item in employee_Advances)
            {
                ...
                bankReportVM.EMP_TRANSACTION_PARTICULARS = "Advance " + wage_Process.WAG_Month.ToString("MMMM") + "-" + wage_Process.WAG_Month.ToString("yyyy");
        ...
        public List<BankReportVM> IDBI_TO_Other_AdvanceReports(int WAG_Id, int FRM_Id)
        {
            ...
            Wage_Process wage_Process = wageProcessManager.getWageProcessById(WAG_Id);
            List<BankReportVM> advanceBankReportVMs = new List<BankReportVM>();
            List<Employee_Advance> employee_Advances = this.GetIDBI_TO_OtherEmployeeAdvances(new DateTime(wage_Process.WAG_Month.Year, wage_Process.WAG_Month.Month, wage_Process.WAG_Month.Day), FRM_Id);

            foreach (var item in employee_Advances)
            {
                BankReportVM bankReportVM = new BankReportVM();
                bankReportVM.EMP_TRANSACTION_AMOUNT = item.ADV_Amount;
                bankReportVM.ACCOUNT_SENDER_NUMBER = item.EMP.FRM?.FRM_AccountNumber;
                bankReportVM.ACCOUNT_IFSC_CODE = item.EMP.EMP_Bank_IFSC;
                bankReportVM.ACCOUNT_RECEIVERS_NUMBER = item.EMP.EMP_Account_Number;
                ...
                bankReportVM.EMP_ADDRESS = item.EMP.EMP_CityNavigation?.CITY_Name;
        ...
        public List<BankReportVM> ChequeCash_AdvancesReports(int WAG_Id, int FRM_Id)
        {
            ...
            List<Employee_Advance> employee_Advances = this.GetCheque_CashEmployeeAdvances(new DateTime(wage_Process.WAG_Month.Year, wage_Process.WAG_Month.Month, wage_Process.WAG_Month.Day), FRM_Id);

            foreach (var item in employee_Advances)
            {
                BankReportVM bankReport = new BankReportVM();

                bankReport.EMP_FirstName = item.EMP.EMP_FirstName;
                bankReport.EMP_MiddleName = item.EMP.EMP_MiddleName;
                bankReport.EMP_SurName = item.EMP.EMP_SurName;
                bankReport.EMP_ADDRESS = item.EMP.EMP_CityNavigation?.CITY_Name;
        ...
        public List<PFClientReportVM> Employees_Pending_For_Registration(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            WageRegisterManager wageManager = new WageRegisterManager(_context);
            List<PFClientReportVM> reportVMs = new List<PFClientReportVM>();
            List<Wage_Register> wage_Registers = wageManager.GetWageRegisters(WAG_Id).Where(m => m.EMP.EMP_UAN_Number == null || m.EMP.EMP_UAN_Number.Equals("Pending") || m.EMP.EMP_UAN_Number.Equals("")).ToList();
            dynamic PFClientList = null;
            if (IsSelected == true)
            {
                int[] CLI_Ids = selectionVMs.Select(m => m.CLI_Id).ToArray();
                ...
                PFClientList = wage.Select(m => new
                {
                    ...
                    m.CLI.CLI_Name,
                    m.EMP.EMP_FirstName,
                    m.EMP.EMP_MiddleName,
                    m.EMP.EMP_SurName,
                    m.EMP.EMP_RegisteredOn,
                    m.EMP.EMP_UAN_Remark
                }).Distinct();
            }
            else
            {
        ...
        public List<PFClientReportVM> Employees_PF_Excel(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            WageRegisterManager wageManager = new WageRegisterManager(_context);
            List<PFClientReportVM> reportVMs = new List<PFClientReportVM>();
            IEnumerable<Wage_Register> wage_Registers = wageManager.GetWageRegistersByWAG_Id(WAG_Id).Where(m => m.EMP.EMP_UAN_Number != null && m.EMP.EMP_UAN_Number != "Pending" && m.EMP.EMP_UAN_Number != "").ToList();
        ...
        public List<PFClientReportVM> Client_Wise_PF_Above58_Excel(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            IEnumerable<Wage_Register> wage_Registers = wageManager.GetWageRegistersByWAG_Id(WAG_Id).Where(m => m.EMP.EMP_UAN_Number != null && m.EMP.EMP_UAN_Number != "Pending" && m.EMP.EMP_UAN_Number != "" && m.EMP.EMP_DOB <= Above57YearDate).ToList();
            List<PFClientReportVM> reportVMs = new List<PFClientReportVM>();
            if (IsSelected == true)
            {
                int[] CLI_Ids = selectionVMs.Select(m => m.CLI_Id).ToArray();
        ...
        public List<NEFT_BankReportVM> NEFT_BankReports(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            foreach (var cli in BankClientList)
            {
                ...
                foreach (var wage in register)
                {
                    ...
                    rptEmployee.EMP_FirstName = wage.EMP.EMP_FirstName;
                    rptEmployee.EMP_MiddleName = wage.EMP.EMP_MiddleName;
                    rptEmployee.EMP_SurName = wage.EMP.EMP_SurName;
                    rptEmployee.EMP_Account_Number = (wage.EMP.EMP_Account_Number == null ? "-" : wage.EMP.EMP_Account_Number);
                    ...
                    rptEmployee.SERVICE_OUTLET = (wage.EMP.EMP_Bank_IFSC == null ? "-" : wage.EMP.EMP_Bank_IFSC);
                    rptEmployee.TRANSACTION_PARTICULARS = "Salary " + wage_Registers.First().WAG.WAG_Month.ToString("MMMM") + "-" + wage_Registers.First().WAG.WAG_Month.ToString("yyyy");
        ...
        public List<BankReportVM> IDBI_TO_IDBI_BankReports(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            foreach (var item in wage_Registers)
            {
                ...
                bankReport.EMP_MiddleName = item.EMP.EMP_MiddleName;
                bankReport.EMP_SurName = item.EMP.EMP_SurName;
                bankReport.EMP_ACCOUNT_NUMBER = item.EMP.EMP_Account_Number;
                ...
                bankReport.EMP_TRANSACTION_PARTICULARS = "Salary " + wage_Registers.First().WAG.WAG_Month.ToString("MMMM") + "-" + wage_Registers.First().WAG.WAG_Month.ToString("yyyy");
                bankReportVMs.Add(bankReport);
            }
            return bankReportVMs;
        }
        ...
        public List<BankReportVM> IDBI_TO_Other_BankReports(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            foreach (var item in wage_Registers)
            {
                BankReportVM bankReport = new BankReportVM();
                bankReport.EMP_TRANSACTION_AMOUNT = (item.WAR_FinalTotal != null ? item.WAR_FinalTotal.Value : 0);
                bankReport.ACCOUNT_SENDER_NUMBER = item.WAG.FRM.FRM_AccountNumber;
                bankReport.ACCOUNT_IFSC_CODE = item.EMP.EMP_Bank_IFSC;
                bankReport.ACCOUNT_RECEIVERS_NUMBER = item.EMP.EMP_Account_Number;
                ...
                if (item.CLI != null)
                    bankReport.CLI_ADDRESS = item.CLI.CLI_Place_Of_Supply;
        ...
        public List<BankReportVM> ChequeCash_BankReports(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            foreach (var item in wage_Registers)
            {
                ...
                bankReport.EMP_MiddleName = item.EMP.EMP_MiddleName;
                bankReport.EMP_SurName = item.EMP.EMP_SurName;
                bankReport.EMP_ADDRESS = item.CLI.CLI_Invoicing_Location;
                bankReport.EMP_TRANSACTION_AMOUNT = (item.WAR_FinalTotal != null ? item.WAR_FinalTotal.Value : 0);
                bankReportVMs.Add(bankReport);
            }
            return bankReportVMs;
        ...
        public List<ESICReportEmpWiseVM> ESICReportEmpWise(int WAG_Id, int FRM_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            ...
            foreach (var client in ESICClientList)
            {
                ...
                {
                    ESICReportVM reportVM = new ESICReportVM();
                    List<Client_Requirement_Allowance> All = register.CRI.Client_Requirement_Allowances.ToList();
                    ...
                    {
                        int days = DateTime.DaysInMonth(register.WAG.WAG_Month.Year, register.WAG.WAG_Month.Month);
                        if (days == 30)
                        {
                            PayableDays = 26;
                        }
            ...
            #region employees
            IEnumerable<Employee> employees = GetLeftEmployeesOfPrevMonth(new DateTime(wage_Registers[0].WAG.WAG_Month.Year, wage_Registers[0].WAG.WAG_Month.Month, wage_Registers[0].WAG.WAG_Month.Day), FRM_Id);
        ...
        public List<ESICReportVM> ESIC_Employees_Pending_For_Registration(int WAG_Id, List<SelectionVM> selectionVMs, bool IsSelected)
        {
            WageRegisterManager wageManager = new WageRegisterManager(_context);
            List<ESICReportVM> reportVMs = new List<ESICReportVM>();
            List<Wage_Register> wage_Registers = wageManager.GetWageRegisters(WAG_Id).Where(m => m.EMP.EMP_ESIC_Number == null || m.EMP.EMP_ESIC_Number.Equals("Pending") || m.EMP.EMP_ESIC_Number.Equals("")).ToList();
            dynamic ESICClientList = null;
            if (IsSelected == true)
            {
                int[] CLI_Ids = selectionVMs.Select(m => m.CLI_Id).ToArray();
