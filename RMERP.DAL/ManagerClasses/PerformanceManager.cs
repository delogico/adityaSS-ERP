...
{   
    public class PerformanceManager
    {
        ...
        public Wage_Register_Performance GetPerformanceById(int WRP_Id)
        {
            ...
            performance = _context.Wage_Register_Performances.Include(M => M.CLE).ThenInclude(M => M.EMP).Where(m=>m.WRP_Id.Equals(WRP_Id)).FirstOrDefault();
            return performance;
        }
        public Wage_Register_Performance GetPerformanceByCLE(int WAG_Id,int CLE_Id,int CLI_Id)
        {
            Wage_Register_Performance performance = new Wage_Register_Performance();
            performance = _context.Wage_Register_Performances.Include(M => M.CLE).ThenInclude(M => M.EMP).Where(m => m.WAG_Id.Equals(WAG_Id) && m.CLE_Id.Equals(CLE_Id) && m.CLE.CLI_Id.Equals(CLI_Id)).FirstOrDefault();
            return performance;
        }
            ...
            try
            {
                foreach(Wage_Register_Performance performance in performances)
                {
                    if (performance.WRP_Id > 0)
                        _context.Wage_Register_Performances.Update(performance);
                    else
                        _context.Wage_Register_Performances.Add(performance);
                }
